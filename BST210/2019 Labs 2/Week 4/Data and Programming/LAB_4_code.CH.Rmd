...
```{r}
set.seed(210)
x.unsort=c(runif(25,0,6),runif(50,6,14),runif(25,14,20))
x=sort(x.unsort)
...
```{r}
#How about something unsorted?
plot(x,y)
lines(x.unsort,running_mean,col="red")
```

Note how when the x's are unsorted, this looks a lot like the errors many people had in their HW1 plots.
...
for(i in 1:100){
  ...
  weight=exp(-(x[neighbor]-x[i])^2)/sum(exp(-(x[neighbor]-x[i])^2)) ## Gaussian Kernel
  kernel_smooth=c(kernel_smooth,sum(weight*y[neighbor]))
  without_weights=c(without_weights,mean(y[neighbor]))
}
plot(x,y)
lines(x,kernel_smooth,col="red")
lines(x,without_weights,col = "blue")
```

Note how the unweighted kernel smoother is the same as the running mean smoother (here shown with a width of 75, like before).
...
Note how things now look different when we change the span. The narrower span makes it pick up more local changes (hot pink curve) and the wide span makes it pick up slightly less of the overall variability. Why does it still follow the curve pretty well even though the span is huge? Because LOWESS is **weighted**, so the far away stuff doesn't affect the curve that much.

```{r}
plot(x,y)
fit=loess(y~x,span=0.4) 
fit2=loess(y~x,span=0.1)
fit3=loess(y~x,span=0.9)
lines(x,fitted(fit),col="cornflowerblue")
lines(x,fitted(fit2),col="deeppink",lwd=2)
lines(x,fitted(fit3),col="mediumpurple4",lwd=2)
```
## Splines
...
```{r}
mod3b=lm(y~bSpline(x,df=4,degree=0,intercept=TRUE))
mod3c=lm(y~bSpline(x,df=3,degree=0))
plot(x,y)
lines(x,fitted(mod3b),type='l',col="mediumpurple4",lwd=2)
lines(x,fitted(mod3c),type='l',col="deeppink")
```


Let's look at this with setting degrees of freedom rather than setting knots. The default for df is to have the knots be evenly-spaced quantiles. Note how the pink line (without the intercept) has some weirdness at the right side. This is because the package makes the start and endpoint the same when degree=0 unless intercept=TRUE. Also note that including an intercept means you have to have one more degree of freedom to have the same number of knots. All of this information is available in the documentation of bSpline, which you can get by typing '?bSpline' into the console.
...
```{r}
mod5=lm(y~bSpline(x,knots=quantile(x,c(0.15,0.5,0.75)),degree=1))
plot(x,y)
lines(x,fitted(mod5),col="red")
```

Note that I moved a knot from the 25th quantile (default when using df rather than knots) to the 15th quantile (by setting it manually).
